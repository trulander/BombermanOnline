...
Проект построен на микросервисной архитектуре. Взаимодействие между сервисами осуществляется через REST API и асинхронную систему обмена сообщениями NATS. Обнаружение сервисов реализовано через Consul. Секреты хранятся в Infisical и подставляются при запуске.
...
graph TD
    ...
    WebFrontend -.->|Секреты| Infisical
    WebAPI -.->|Секреты| Infisical
    GameService -.->|Секреты| Infisical
    AuthService -.->|Секреты| Infisical
    GameAllocator -.->|Секреты| Infisical
    AIService -.->|Секреты| Infisical
...
*   **[Infisical](docs/ru/infra/infisical/index.md)** (Управление секретами)
*   **Экспортеры метрик**
...
Универсальный сервис-распределитель, отвечающий за эффективное распределение нагрузки по доступным экземплярам сервисов (`Game Service`, `AI Service` и других). Он отслеживает текущую нагрузку (CPU и RAM) через Prometheus, определяет группу равнонагруженных инстансов и использует round-robin ротацию для равномерного распределения запросов. Сервис автоматически регистрирует обработчики для всех настроенных сервисов через конфигурацию, обеспечивая масштабируемость и отказоустойчивость.
...
Секреты хранятся в Infisical. В каждом сервисе есть файл `.env-example` со списком доступных переменных, который можно импортировать в Infisical как базовую конфигурацию. Docker entrypoint-скрипты входят в Infisical и запускают сервисы с подставленными переменными окружения.
...
-   **Игра и Frontend**: `http://bomberman.localhost` или `http://localhost`
...
- Infisical (Управление секретами)
