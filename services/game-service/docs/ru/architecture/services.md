...
-   **Генерация случайных карт (`generate_random_map`)**: 
    ...
    -   Размещает точки спавна игроков (`_add_player_spawns`), используя метод `_generate_spawn_positions` для генерации позиций. Метод поддерживает различные стратегии генерации:
        -   Использование углов карты как приоритетных позиций (настраивается через `use_corner_spawns`)
        -   Генерация большего количества spawn точек, чем игроков (через `spawn_points_count`), для большей рандомизации
        -   При включенной настройке `randomize_spawn_positions` позиции spawn точек перемешиваются перед размещением на карте
