...
-   **gRPC тренировка AI:** Отдельный режим обучения с `reset/step` без общего игрового цикла.
...
Сервис настраивается с помощью переменных окружения. Пример файла с переменными окружения можно найти в `services/game-service/.env-example`. Этот файл можно импортировать в Infisical как базовую конфигурацию. Основные переменные:
...
-   `AI_SERVICE_GRPC_HOST`, `AI_SERVICE_GRPC_PORT`: gRPC адрес ai-service для инференса AI-юнитов.
...
-   `AI_ACTION_INTERVAL`: Интервал между запросами инференса для AI (секунды).

## Управление секретами

Секреты хранятся в Infisical. Docker entrypoint входит в Infisical через переменные `INFISICAL_*` и запускает сервис с подставленными переменными окружения. Файл `.env-example` можно использовать для базовой загрузки параметров.
...
- `game-service.postman_environment.json` — окружение

-   `Reset` создает отдельную тренировочную игру и возвращает `session_id` и наблюдение.
-   `Step` выполняет расчет на `delta_seconds` (по умолчанию 0.33 сек) и возвращает наблюдение, `reward`, `terminated`, `truncated`.
-   Тренировочные игры не попадают в общий список и не используют глобальный игровой цикл.
-   Для сущностей с `ai=True` в обычном цикле выполняется инференс в `ai-service`.
