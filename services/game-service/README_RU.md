...
-   **Система движения игроков:** Непрерывное движение по сетке с учётом коллизий и «умным поворотом» (corner assist), который автоматически слегка выравнивает игрока по полосе коридора, если он уже вышел хотя бы на половину клетки и пытается повернуть.
...
-   `AI_INFERENCE_TIMEOUT_SEC`: Таймаут gRPC-вызова инференса (секунды).
-   `AI_INFERENCE_BACKOFF_INITIAL_SEC`, `AI_INFERENCE_BACKOFF_STEP_SEC`, `AI_INFERENCE_BACKOFF_MAX_SEC`: параметры паузы (backoff) после повторных ошибок инференса — задержка растёт до максимума (по умолчанию 3 с), чтобы не опрашивать недоступный ai-service слишком часто.

**Устойчивость инференса при недоступности ai-service:** используется один общий gRPC-канал на весь сервис; все AI-сущности делят его. При падении ai-service: запросы ограничены таймаутом; при ошибках выполняется «мягкий» сброс канала (без блокирующего закрытия), сброс кеша инстанса и повторное получение инстанса при следующем подключении; действует backoff до 3 с при повторных ошибках, чтобы не нагружать недоступный сервис.
