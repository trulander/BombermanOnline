...
import { Map } from '../types/GameState';
...
export class Renderer {
    ...
    // Новые методы для работы с обновленным клиентом
    ...
    public drawMenu(): void {
        ...
        this.ctx.textAlign = 'center';
        this.ctx.fillText('BOMBERMAN ONLINE', this.canvas.width / 2, this.canvas.height / 2 - 80);
        
        // Создаем кнопки
        this.createButton('CREATE GAME', this.canvas.width / 2, this.canvas.height / 2, 200, 50, 'create-game-btn');
        this.createButton('JOIN GAME', this.canvas.width / 2, this.canvas.height / 2 + 70, 200, 50, 'join-game-btn');
    }
    
    private createButton(text: string, x: number, y: number, width: number, height: number, id: string): void {
        // Создаем HTML кнопку и добавляем ее поверх canvas
    ...
    public drawJoinGameForm(): void {
        // Очищаем canvas и удаляем старые кнопки
        this.clearCanvas();
        this.removeButtons();
        ...
        this.ctx.textAlign = 'center';
        this.ctx.fillText('JOIN GAME', this.canvas.width / 2, this.canvas.height / 2 - 80);
        
        // Создаем форму ввода ID игры
        const formContainer = document.createElement('div');
        formContainer.id = 'join-game-form';
        formContainer.style.position = 'absolute';
        formContainer.style.left = `${this.canvas.width / 2 - 150}px`;
        formContainer.style.top = `${this.canvas.height / 2 - 30}px`;
        ...
        joinButton.style.backgroundColor = '#555';
        ...
        if (canvasContainer) {
            canvasContainer.style.position = 'relative';
            canvasContainer.appendChild(formContainer);
        } else {
    ...
    private removeButtons(): void {
        // Удаляем все кнопки и формы
        document.getElementById('create-game-btn')?.remove();
        document.getElementById('join-game-btn')?.remove();
        document.getElementById('join-game-form')?.remove();
    }
    
    public drawText(text: string, x: number, y: number, color: string = 'white'): void {
        this.ctx.fillStyle = color;
        this.ctx.font = '24px Arial';
        this.ctx.textAlign = 'center';
    ...
    public drawGameOver(score: number, level: number): void {
        // Очищаем canvas и удаляем все кнопки
        this.clearCanvas();
        this.removeButtons();
